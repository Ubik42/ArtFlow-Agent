...
OUTPUT_ROOT = ROOT / "artifacts/goal/m39-s1-live-camera-move"
...
def main() -> int:
    ...
    if first.json()["scene_dcc_work"] != repeated.json()["scene_dcc_work"]:
        raise RuntimeError("repeated camera-move dispatch changed the registered work")
    ...
    if work["status"] != "succeeded":
        raise RuntimeError(f"camera-move work stopped at {work['status']}: {work['message']}")
    if len(work["definition"]["capability_ids"]) != 12:
        raise RuntimeError("camera move was not registered as the twelfth capability")
    ...
    result = {
        "schema_id": "artflow-live-camera-move-work-receipt/1",
        "run_id": RUN_ID,
        ...
        "camera_move_request_sha256": work["definition"]["camera_move_request_sha256"],
        "blender_camera_move_receipt_sha256": work["definition"][
            "blender_camera_move_receipt_sha256"
        ],
        "unreal_camera_move_receipt_sha256": work["definition"][
            "unreal_camera_move_receipt_sha256"
        ],
        "camera_move_sequence_path": work["definition"]["camera_move_sequence_path"],
    ...
    (OUTPUT_ROOT / "live-camera-move-work-receipt.json").write_text(
