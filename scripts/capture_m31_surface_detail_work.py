...
OUTPUT_ROOT = ROOT / "artifacts/goal/m31-s1-live-surface-detail"
...
def main() -> int:
    ...
    if first.json()["scene_dcc_work"] != repeated.json()["scene_dcc_work"]:
        raise RuntimeError("repeated surface-detail dispatch changed the registered work")
    ...
    if work["status"] != "succeeded":
        raise RuntimeError(f"surface-detail work stopped at {work['status']}: {work['message']}")
    if len(work["definition"]["capability_ids"]) != 8:
        raise RuntimeError("surface detail was not registered as the eighth DCC capability")
    ...
    result = {
        "schema_id": "artflow-live-surface-detail-work-receipt/1",
        "run_id": RUN_ID,
        ...
        "surface_detail_request_sha256": work["definition"]["surface_detail_request_sha256"],
        "comfy_surface_detail_receipt_sha256": work["definition"]["comfy_surface_detail_receipt_sha256"],
        "blender_surface_detail_receipt_sha256": work["definition"]["blender_surface_detail_receipt_sha256"],
    ...
    (OUTPUT_ROOT / "live-surface-detail-work-receipt.json").write_text(
