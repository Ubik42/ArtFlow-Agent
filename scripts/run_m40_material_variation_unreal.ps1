...
$script = Join-Path $repository "integrations\unreal\apply_material_variation_candidate.py"
...
[pscustomobject]@{
    ...
    prior_pids = $before
    surviving_prior_pids = @($before | Where-Object { $after -contains $_ })
    log_path = $resolvedLog
} | ConvertTo-Json -Compress
if ($process.ExitCode -ne 0) { exit $process.ExitCode }
$receipt = Join-Path $repository "artifacts\goal\m40-s1-material-variation\unreal-material-variation-receipt.json"
if (-not (Test-Path -LiteralPath $receipt)) {
    throw "Material variation receipt was not produced; inspect $resolvedLog"
}
