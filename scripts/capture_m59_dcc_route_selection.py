...
OUTPUT_ROOT = ROOT / "artifacts/goal/m59-s1-dcc-route-selection"
EXPECTED_ROUTE = [
...
def main() -> int:
    ...
    if first.json()["scene_dcc_work"] != repeated.json()["scene_dcc_work"]:
        raise RuntimeError("repeated route dispatch changed the registered work")
    ...
    if work["status"] != "succeeded" or definition["capability_ids"] != EXPECTED_ROUTE:
        raise RuntimeError(f"routed DCC work stopped at {work['status']}: {work['message']}")
    if decision["selected_route_id"] != "cloth_banner":
        raise RuntimeError(f"unexpected selected route: {decision['selected_route_id']}")
    ...
    result = {
        "schema_id": "artflow-dcc-route-selection-receipt/1",
        "run_id": RUN_ID,
        ...
        "dispatched_capability_ids": definition["capability_ids"],
        "dispatched_stage_count": len(definition["capability_ids"]),
        "verified_capability_count": 21,
        ...
        "replay_equivalent": restored is not None
        and restored.model_dump(mode="json") == work,
    }
    (OUTPUT_ROOT / "dcc-route-selection-receipt.json").write_text(
