...
$script = (Join-Path $repository "integrations\unreal\apply_modular_environment_candidate.py").Replace("\", "/")
...
$receipt = Join-Path $repository "artifacts\goal\m47-s1-modular-environment\unreal-modular-environment-receipt.json"
if (-not (Test-Path -LiteralPath $receipt)) { throw "Modular receipt was not produced; inspect $resolvedLog" }
