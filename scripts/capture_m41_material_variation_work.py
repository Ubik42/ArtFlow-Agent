...
OUTPUT_ROOT = ROOT / "artifacts/goal/m41-s1-live-material-variation"
...
def main() -> int:
    ...
    if first.json()["scene_dcc_work"] != repeated.json()["scene_dcc_work"]:
        raise RuntimeError("repeated material dispatch changed the registered work")
    ...
    if work["status"] != "succeeded":
        raise RuntimeError(f"material work stopped at {work['status']}: {work['message']}")
    if len(work["definition"]["capability_ids"]) != 13:
        raise RuntimeError("material variation was not registered as capability thirteen")
    ...
    result = {
        "schema_id": "artflow-live-material-variation-work-receipt/1",
        "run_id": RUN_ID,
        ...
        "material_variation_request_sha256": work["definition"][
            "material_variation_request_sha256"
        ],
        "blender_material_variation_receipt_sha256": work["definition"][
            "blender_material_variation_receipt_sha256"
        ],
        "unreal_material_variation_receipt_sha256": work["definition"][
            "unreal_material_variation_receipt_sha256"
        ],
    ...
    (OUTPUT_ROOT / "live-material-variation-work-receipt.json").write_text(
