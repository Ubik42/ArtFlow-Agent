...
OUTPUT_ROOT = ROOT / "artifacts/goal/m46-s1-live-foliage"
...
def main() -> int:
    ...
    client = TestClient(create_app(runs_dir=OUTPUT_ROOT, agent_database=database, project_root=ROOT), client=("127.0.0.1", 54610))
    ...
    if first.json()["scene_dcc_work"] != repeated.json()["scene_dcc_work"]:
        raise RuntimeError("repeated foliage dispatch changed the registered work")
    ...
    if work["status"] != "succeeded" or len(work["definition"]["capability_ids"]) != 15:
        raise RuntimeError(f"foliage work stopped at {work['status']}: {work['message']}")
    ...
    result = {
        "schema_id": "artflow-live-biome-foliage-work-receipt/1", "run_id": RUN_ID,
        ...
        "foliage_kit_request_sha256": work["definition"]["foliage_kit_request_sha256"],
        "blender_foliage_kit_receipt_sha256": work["definition"]["blender_foliage_kit_receipt_sha256"],
        "unreal_foliage_kit_receipt_sha256": work["definition"]["unreal_foliage_kit_receipt_sha256"],
    ...
    (OUTPUT_ROOT / "live-biome-foliage-work-receipt.json").write_text(json.dumps(result, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
