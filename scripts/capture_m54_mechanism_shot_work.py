...
OUTPUT_ROOT = ROOT / "artifacts/goal/m54-s1-live-mechanism-shot"
...
def main() -> int:
    ...
    if first.json()["scene_dcc_work"] != repeated.json()["scene_dcc_work"]:
        raise RuntimeError("repeated mechanism-shot dispatch changed the registered work")
    ...
    if work["status"] != "succeeded" or len(work["definition"]["capability_ids"]) != 19:
        raise RuntimeError(f"mechanism-shot work stopped at {work['status']}: {work['message']}")
    ...
    result = {
        "schema_id": "artflow-live-mechanism-shot-work-receipt/1",
        "run_id": RUN_ID,
        ...
        "mechanism_shot_request_sha256": definition["mechanism_shot_request_sha256"],
        "unreal_mechanism_shot_receipt_sha256": definition[
            "unreal_mechanism_shot_receipt_sha256"
        ],
        "mechanism_shot_sequence_path": definition["mechanism_shot_sequence_path"],
        "preview_sha256s": {
            "closed_start": definition["mechanism_shot_closed_start_sha256"],
            "open": definition["mechanism_shot_open_sha256"],
            "closed_end": definition["mechanism_shot_closed_end_sha256"],
        },
    ...
    (OUTPUT_ROOT / "live-mechanism-shot-work-receipt.json").write_text(
