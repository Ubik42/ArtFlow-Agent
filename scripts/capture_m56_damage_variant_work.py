...
OUTPUT_ROOT = ROOT / "artifacts/goal/m56-s1-live-damage-variant"
...
def main() -> int:
    ...
    if first.json()["scene_dcc_work"] != repeated.json()["scene_dcc_work"]:
        raise RuntimeError("repeated damage dispatch changed the registered work")
    ...
    if work["status"] != "succeeded" or len(work["definition"]["capability_ids"]) != 20:
        raise RuntimeError(
            f"damage work stopped at {work['status']}: {work['message']}"
        )
    events = AgentEventStore(database).events(RUN_ID)
    dcc_events = [
        event for event in events if event.event_type.startswith("scene_dcc_work_")
    ]
    restored = AgentEventStore(database).load(RUN_ID).scene_dcc_work
    definition = work["definition"]
    result = {
        "schema_id": "artflow-live-damage-variant-work-receipt/1",
        "run_id": RUN_ID,
        ...
        "damage_variant_request_sha256": definition[
            "damage_variant_request_sha256"
        ],
        "host_receipt_sha256s": {
            "comfyui": definition["comfy_damage_field_receipt_sha256"],
            "blender": definition["blender_damage_variant_receipt_sha256"],
            "unreal": definition["unreal_damage_variant_receipt_sha256"],
        },
        "artifact_sha256s": {
            "field": definition["damage_field_sha256"],
            "glb": definition["damage_glb_sha256"],
            "material_mask": definition["damage_material_mask_sha256"],
            "manifest": definition["damage_manifest_sha256"],
            "blender_preview": definition["damage_blender_preview_sha256"],
            "unreal_preview": definition["damage_unreal_preview_sha256"],
        },
        ...
        "replay_equivalent": (
            restored is not None and restored.model_dump(mode="json") == work
        ),
    }
    (OUTPUT_ROOT / "live-damage-variant-work-receipt.json").write_text(
