...
$script = (Join-Path $repository "integrations\unreal\apply_biome_foliage_candidate.py").Replace("\", "/")
...
$receipt = Join-Path $repository "artifacts\goal\m45-s1-foliage-kit\unreal-biome-foliage-receipt.json"
if (-not (Test-Path -LiteralPath $receipt)) { throw "Foliage receipt was not produced; inspect $resolvedLog" }
