...
OUTPUT_ROOT = ROOT / "artifacts/goal/m33-s1-live-procedural-kit"
...
def main() -> int:
    ...
    if first.json()["scene_dcc_work"] != repeated.json()["scene_dcc_work"]:
        raise RuntimeError("repeated procedural-kit dispatch changed the registered work")
    ...
    if work["status"] != "succeeded":
        raise RuntimeError(f"procedural-kit work stopped at {work['status']}: {work['message']}")
    if len(work["definition"]["capability_ids"]) != 9:
        raise RuntimeError("procedural kit was not registered as the ninth DCC capability")
    ...
    result = {
        "schema_id": "artflow-live-procedural-kit-work-receipt/1",
        "run_id": RUN_ID,
        ...
        "procedural_kit_request_sha256": work["definition"]["procedural_kit_request_sha256"],
        "procedural_kit_receipt_sha256": work["definition"]["procedural_kit_receipt_sha256"],
        "procedural_kit_manifest_sha256": work["definition"]["procedural_kit_manifest_sha256"],
    ...
    (OUTPUT_ROOT / "live-procedural-kit-work-receipt.json").write_text(
