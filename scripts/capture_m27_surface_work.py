...
OUTPUT_ROOT = ROOT / "artifacts/goal/m27-s1-live-surface"
...
def main() -> int:
    ...
    if first.json()["scene_dcc_work"] != repeated.json()["scene_dcc_work"]:
        raise RuntimeError("repeated surface dispatch changed the registered work")
    ...
    if work["status"] != "succeeded":
        raise RuntimeError(f"surface work stopped at {work['status']}: {work['message']}")
    ...
    result = {
        "schema_id": "artflow-live-surface-work-receipt/1",
        "run_id": RUN_ID,
        ...
        "surface_request_sha256": work["definition"]["surface_request_sha256"],
    ...
    (OUTPUT_ROOT / "live-surface-work-receipt.json").write_text(
