...
OUTPUT_ROOT = ROOT / "artifacts/goal/m29-s1-live-dressing"
...
def main() -> int:
    ...
    if first.json()["scene_dcc_work"] != repeated.json()["scene_dcc_work"]:
        raise RuntimeError("repeated set-dressing dispatch changed the registered work")
    ...
    if work["status"] != "succeeded":
        raise RuntimeError(f"set-dressing work stopped at {work['status']}: {work['message']}")
    if len(work["definition"]["capability_ids"]) != 7:
        raise RuntimeError("set dressing was not registered as the seventh DCC capability")
    ...
    result = {
        "schema_id": "artflow-live-set-dressing-work-receipt/1",
        "run_id": RUN_ID,
        ...
        "set_dressing_request_sha256": work["definition"]["set_dressing_request_sha256"],
        "set_dressing_manifest_sha256": work["definition"]["set_dressing_manifest_sha256"],
    ...
    (OUTPUT_ROOT / "live-set-dressing-work-receipt.json").write_text(
