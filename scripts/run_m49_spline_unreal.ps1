...
$script = (Join-Path $repository "integrations\unreal\apply_spline_infrastructure_candidate.py").Replace("\", "/")
...
$receipt = Join-Path $repository "artifacts\goal\m49-s1-spline-infrastructure\unreal-spline-infrastructure-receipt.json"
if (-not (Test-Path -LiteralPath $receipt)) { throw "Spline receipt was not produced; inspect $resolvedLog" }
