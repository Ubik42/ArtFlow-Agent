...
$script = (Join-Path $repository "integrations\unreal\apply_cloth_banner_candidate.py").Replace("\", "/")
...
$receipt = Join-Path $repository "artifacts\goal\m57-s1-cloth-banner\unreal-cloth-banner-receipt.json"
if (-not (Test-Path -LiteralPath $receipt)) { throw "Cloth banner receipt was not produced; inspect $resolvedLog" }
