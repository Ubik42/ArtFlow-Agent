...
OUTPUT_ROOT = ROOT / "artifacts/goal/m37-s1-live-shot-package"
...
def main() -> int:
    ...
    if first.json()["scene_dcc_work"] != repeated.json()["scene_dcc_work"]:
        raise RuntimeError("repeated shot-package dispatch changed the registered work")
    ...
    if work["status"] != "succeeded":
        raise RuntimeError(f"shot-package work stopped at {work['status']}: {work['message']}")
    if len(work["definition"]["capability_ids"]) != 11:
        raise RuntimeError("shot packaging was not registered as the eleventh capability")
    ...
    result = {
        "schema_id": "artflow-live-shot-package-work-receipt/1",
        "run_id": RUN_ID,
        ...
        "shot_package_request_sha256": work["definition"]["shot_package_request_sha256"],
        "unreal_shot_package_receipt_sha256": work["definition"][
            "unreal_shot_package_receipt_sha256"
        ],
        "level_sequence_path": work["definition"]["level_sequence_path"],
    ...
    (OUTPUT_ROOT / "live-shot-package-work-receipt.json").write_text(
