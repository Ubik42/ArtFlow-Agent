...
$script = Join-Path $repository "integrations\unreal\apply_native_pcg_density_candidate.py"
...
$receipt = Join-Path $repository "artifacts\goal\m34-s1-pcg-density\unreal-native-pcg-density-receipt.json"
if (-not (Test-Path -LiteralPath $receipt)) { throw "Native PCG receipt was not produced; inspect $resolvedLog" }
