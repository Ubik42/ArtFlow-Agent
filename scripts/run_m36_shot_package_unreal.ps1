...
$script = Join-Path $repository "integrations\unreal\apply_shot_package_candidate.py"
...
$receipt = Join-Path $repository "artifacts\goal\m36-s1-shot-package\unreal-shot-package-receipt.json"
if (-not (Test-Path -LiteralPath $receipt)) { throw "Shot package receipt was not produced; inspect $resolvedLog" }
