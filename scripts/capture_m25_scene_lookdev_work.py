...
OUTPUT_ROOT = ROOT / "artifacts/goal/m25-s1-live-lookdev"
...
def main() -> int:
    ...
    client = TestClient(
        ...
        client=("127.0.0.1", 52510),
    )
    base = f"/api/agent/runs/{RUN_ID}/scene-dcc-work"
    first = client.post(f"{base}/queue")
    second = client.post(f"{base}/queue")
    first.raise_for_status()
    second.raise_for_status()
    if first.json()["scene_dcc_work"] != second.json()["scene_dcc_work"]:
        raise RuntimeError("repeated lookdev dispatch changed the registered work")
    ...
    if work["status"] != "succeeded":
        raise RuntimeError(f"lookdev work stopped at {work['status']}: {work['message']}")
    ...
    restored_once = AgentEventStore(database).load(RUN_ID).scene_dcc_work
    restored_twice = AgentEventStore(database).load(RUN_ID).scene_dcc_work
    result = {
        "schema_id": "artflow-live-lookdev-work-receipt/1",
        "run_id": RUN_ID,
        ...
        "accepted_visual_target_sha256": work["definition"]["accepted_visual_target_sha256"],
        ...
        "replay_equivalent": restored_once == restored_twice
        and restored_once is not None
        and restored_once.model_dump(mode="json") == work,
    }
    (OUTPUT_ROOT / "live-lookdev-work-receipt.json").write_text(
