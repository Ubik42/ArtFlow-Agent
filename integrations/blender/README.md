...
Agent 选择能力并填写规格，不提交 Blender Python。`generate_architectural_prop.py` 是随项目版本化的固定建模执行器；`assemble_comfy_pbr.py` 读取内容哈希绑定的建模回执和已验证 ComfyUI PBR 回执，把 Base Color、DirectX Normal 与 Roughness 装配到声明的 Blender 材质槽。`generate_geometry_layout.py` 再根据 Session 边界、数量、变化幅度和种子建立固定 Geometry Nodes 图，保留可编辑实例布局，并为 Unreal 实现为单一交换资产。
...
后续能力会复用同一 DCC 工作生命周期增加烘焙、相机灯光交换与 Alembic 模拟缓存，而不是为每类 Blender 功能另建状态系统。
